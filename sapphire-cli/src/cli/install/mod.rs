// FILE: sapphire-cli/src/cli/install/mod.rs
...
struct InstallPlanInput {
    ...
    initial_errors: Vec<(String, SapphireError)>,
}
    ...
    match cache.load_raw(filename) {
        Ok(data) => {
            ...
            serde_json::from_str(&data).map_err(|e| {
                ...
                SapphireError::Cache(format!("Failed parse cached {filename}: {e}"))
            })
        }
        Err(_) => {
...
impl Install {
    pub async fn run(&self, cfg: &Config, cache: Arc<Cache>) -> Result<()> {
        ...
        if !formula_list_to_resolve.is_empty() {
            ...
            match resolver.resolve_targets(&formula_list_to_resolve) {
                ...
                Err(e) => {
                    ...
                    return Err(SapphireError::InstallError(msg));
                }
            }
        } else {
            ...
            for dep in &graph.install_plan {
                    ...
                    .any(|e| e.contains(&format!("'{}'", dep.formula.name())))
                {
                    let needs_source_build = self.build_from_source
                        || !sapphire_core::build::formula::has_bottle_for_current_platform(
                            &dep.formula,
                        );
            ...
            if js.is_empty() && pending_tasks > 0 {
                ...
                match js.join_next().await {
                    Some(Ok((name, result))) => {
                        ...
                        if let Err(SapphireError::InstallError(ref msg)) = result {
        ...
        for (name, result) in task_results {
            match result {
                ...
                Err(e) => {
                    if !matches!(&e, SapphireError::InstallError(msg) if msg.contains("already installed"))
                    {
    ...
    async fn gather_full_dependency_set(
        ...
        if self.cask_only {
            ...
            for name in &self.names {
                if known_formulae.contains(name) {
                    ...
                    plan_input
                        .initial_errors
                        .push((name.clone(), SapphireError::NotFound(msg)));
                }
            }
        }
        ...
        let mut cask_fetch_errors: Vec<(String, SapphireError)> = Vec::new();
            ...
            match api::get_cask(&token).await {
                ...
                Err(SapphireError::NotFound(_)) => {
                    ...
                    if !cask_fetch_errors.iter().any(|(n, _)| n == &token) {
                        ...
                        cask_fetch_errors
                            .push((token.clone(), SapphireError::NotFound(msg.clone())));
                    }
                }
                Err(e) => {
