...
async fn install_formulae(
    ...
    while !nodes.values().all(|n| matches!(n.state, InstallState::Ok(_) | InstallState::Failed(_))) {
    ...
    // Final Check
    let failures: Vec<_> = nodes.iter().filter_map(|(n, node)| match &node.state {
    ...
    if failures.is_empty() {
        ...
        for (pkg, msg) in &failures { error!("  ✖ {}: {}", pkg, msg); }
        Err(SapphireError::InstallError(format!("{} bottle(s) failed to install.", failures.len())))
    }
}
