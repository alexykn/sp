...
pub async fn run_uninstall(names: &[String], config: &Config, cache: Arc<Cache>) -> Result<()> {
    ...
    for name in names {
        ...
        if is_formula {
            ...
            if !cellar_path.exists() {
                log::warn!("Formula '{}' info found but keg missing: {}. Attempting cleanup.", name, cellar_path.display());
                match build::formula::link::unlink_formula_artifacts(&formula, config) {
                    Ok(_) => pb.finish_with_message(format!("Unlinked remaining artifacts for {} (keg missing)", name)),
                    Err(e) => {
                        log::error!("Failed to unlink artifacts for missing keg {}: {}", formula.name(), e);
                        errors.push((name.to_string(), SapphireError::NotFound(format!("Formula '{}' is not installed (no keg at {})", name, cellar_path.display()))));
                        pb.finish_and_clear();
                    }
                }
                continue;
            ...
            match build::formula::link::unlink_formula_artifacts(&formula, config) {
                ...
                Err(e) => {
                    log::error!("Failed to unlink artifacts for {}: {}. Proceeding with keg removal.", formula.name(), e);
                    unlink_error = Some(e);
                }
            }
        ...
        match api::fetch_cask(name).await { // Fetch JSON definition
            Ok(cask_json) => {
                ...
                let installed_version = match cask.installed_version(config) {
                    ...
                    None => {
                        ...
                        if errors.iter().all(|(n, e)| n != name || !matches!(e, SapphireError::NotFound(_))) {
                           // Only add error if it wasn't already marked as not found
                            errors.push((name.to_string(), SapphireError::NotFound(format!("Cask '{}' is not installed", name))));
                        }
                        pb.finish_and_clear(); continue;
                    }
                };
                ...
                if !cask_version_path.exists() {
                    log::error!("Cask '{}' version '{}' inconsistent: Dir missing: {}", name, installed_version, cask_version_path.display());
                    errors.push((name.to_string(), SapphireError::NotFound(format!("Cask '{}' version '{}' installation is inconsistent (missing dir)", name, installed_version))));
                    pb.finish_and_clear(); continue;
                }

                let (file_count, size_bytes) = count_files_and_size(&cask_version_path).unwrap_or((0, 0));
                    ...
                    match fs::read_to_string(&manifest_path) {
                        Ok(manifest_str) => {
                            match serde_json::from_str::<CaskInstallManifest>(&manifest_str) {
                                Ok(manifest) => {
                                    for artifact in manifest.artifacts.iter().rev() { // Iterate backward for safety
                                ...
                                Err(e) => {
                                    ...
                                    errors.push((name.to_string(), SapphireError::Generic(format!("Failed parse manifest for cask '{}'", name))));
                                     artifact_removal_errors +=1; // Count as error if manifest is unparseable
                                }
                            }
                        }
                        Err(e) => {
                            log::warn!("Failed to read cask manifest {}: {}. Falling back to directory removal only.", manifest_path.display(), e);
                             errors.push((name.to_string(), SapphireError::Generic(format!("Failed read manifest for cask '{}'", name))));
                             artifact_removal_errors +=1; // Count as error if manifest cannot be read
                        }
                    }
                } else {
                    log::warn!("No CASK_INSTALL_MANIFEST.json found for cask {}. Uninstalling Caskroom directory only.", name);
                     // This is not necessarily an error, just a limitation. Don't increment artifact_removal_errors here.
                }
                ...
                if let Err(e) = fs::remove_dir_all(&cask_version_path) {
                    log::error!("Failed to remove cask version directory {}: {}", cask_version_path.display(), e);
                    errors.push((name.to_string(), SapphireError::Io(std::io::Error::new(e.kind(), format!("Failed to remove cask version dir for '{}'", name)))));
                ...
                // --- Final Message ---
                if artifact_removal_errors == 0 && errors.iter().all(|(n, _)| n != name) { // Check errors specifically for this cask
                    pb.finish_with_message(format!("Uninstalled {} (~{} files, ~{})", cask_version_path.display(), file_count, format_size(size_bytes)));
                } else {
                    log::warn!("Uninstalled {} but encountered {} artifact removal errors.", cask_version_path.display(), artifact_removal_errors);
                    // Add generic error if needed
                     if errors.iter().all(|(n, _)| n != name) {
                         errors.push((name.to_string(), SapphireError::Generic(format!("Encountered {} errors removing artifacts for cask '{}'", artifact_removal_errors, name))));
                     }
                    pb.finish_and_clear();
                }
            ...
            Err(SapphireError::NotFound(_)) => {
                log::error!("Formula or Cask '{}' not found.", name);
                 ...
                 if errors.iter().all(|(n, e)| n != name || !matches!(e, SapphireError::NotFound(_))) {
                     errors.push((name.to_string(), SapphireError::NotFound(format!("Formula or Cask '{}' not found", name))));
                 }
                pb.finish_and_clear(); continue;
            }
            Err(e) => {
    ...
    if !errors.is_empty() {
        ...
        let mut errors_by_pkg: std::collections::HashMap<String, Vec<String>> = std::collections::HashMap::new();
        for (pkg_name, error) in errors {
            errors_by_pkg.entry(pkg_name).or_default().push(error.to_string());
        }
        ...
        return Err(SapphireError::Generic(format!("Uninstall failed for one or more packages.")));
    }
...
fn process_artifact_uninstall(artifact: &InstalledArtifact) -> bool {
    ...
    match artifact {
        ...
        InstalledArtifact::BinaryLink { link_path, .. } => {
            remove_filesystem_artifact(link_path, false) // No sudo usually needed for links in PREFIX
        }
...
fn remove_filesystem_artifact(path: &Path, use_sudo: bool) -> bool {
     match path.symlink_metadata() {
        Ok(metadata) => {
            ...
            log::debug!("Removing {} at: {}", if is_dir {"directory"} else {"file/symlink"}, path.display());
            let remove_result = if is_dir { fs::remove_dir_all(path) } else { fs::remove_file(path) };

            if let Err(e) = remove_result {
                if use_sudo && (e.kind() == std::io::ErrorKind::PermissionDenied || e.kind() == std::io::ErrorKind::DirectoryNotEmpty) {
                    log::warn!("Direct removal failed ({}). Trying with sudo rm -rf...", e);
                    let output = Command::new("sudo").arg("rm").arg("-rf").arg(path).output();
                    match output {
                        Ok(out) if out.status.success() => { log::info!("Successfully removed {} with sudo.", path.display()); true }
                        Ok(out) => { log::error!("Failed to remove {} with sudo: {}", path.display(), String::from_utf8_lossy(&out.stderr)); false }
                        Err(sudo_err) => { log::error!("Error executing sudo rm for {}: {}", path.display(), sudo_err); false }
                    }
                } else {
        ...
        Err(e) => {
            log::warn!("Failed to get metadata for artifact {}: {}", path.display(), e);
...
fn forget_pkgutil_receipt(id: &str) -> bool {
    ...
    match output {
        ...
        Err(e) => { log::error!("Failed to execute sudo pkgutil --forget {}: {}", id, e); false }
    }
}
...
fn unload_and_remove_launchd(label: &str, path: Option<&Path>) -> bool {
    ...
    let unload_output = Command::new("sudo").arg("launchctl").arg("unload").arg("-w").arg(label).output();
     match unload_output {
         Ok(out) => {
             if !out.status.success() {
                let stderr = String::from_utf8_lossy(&out.stderr);
         ...
         Err(e) => { log::error!("Failed to execute sudo launchctl unload for {}: {}", label, e); success = false; }
     }

    // Remove the plist file if path is provided
    if let Some(plist_path) = path {
         log::debug!("Attempting removal of launchd plist: {}", plist_path.display());
         // Use the helper, assuming plist might need sudo depending on location
         if !remove_filesystem_artifact(plist_path, true) {
             log::warn!("Failed to remove launchd plist file: {}", plist_path.display());
            ...
            Ok(mut entries) => {
                if entries.next().is_none() { // Check if directory is empty
                    ...
                    if let Err(e) = std::fs::remove_dir(parent_cask_dir) {
                     ...
                     log::debug!("Parent cask directory {} is not empty, skipping removal.", parent_cask_dir.display());
                }
            },
