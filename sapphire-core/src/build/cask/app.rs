...
/// Returns a Vec containing the details of artifacts created.
pub fn install_app_from_staged(
    ...
    if !move_output.status.success() {
        ...
        if stderr.contains("cross-device link") || stderr.contains("operation not permitted") || stderr.contains("permission denied") {
    ...
    let mut created_artifacts = vec![
        InstalledArtifact::App { path: final_app_destination.clone() }
    ];
