                        ...
                        match key.as_str() {
                            "pkgutil" => {
                                if let Some(id) = val.as_str() {
                                    artifacts.push(InstalledArtifact::PkgUtilReceipt { id: id.to_string() });
                                }
                            }
                            "delete" => {
                                if let Some(arr) = val.as_array() {
                                    for p in arr.iter().filter_map(|v| v.as_str()) {
                                        artifacts.push(
                                            InstalledArtifact::App { path: PathBuf::from(p) }
                                        );
                                    }
                                }
                            }
                            ...
                            "launchctl" => {
                                if let Some(arr) = val.as_array() {
                                    for lbl in arr.iter().filter_map(|v| v.as_str()) {
                                        artifacts.push(
                                            InstalledArtifact::Launchd { label: lbl.to_string(), path: None }
                                        );
                                    }
                                }
                            }
