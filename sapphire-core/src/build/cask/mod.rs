...
pub async fn download_cask(cask: &Cask, cache: &Cache) -> Result<PathBuf> {
    ...
    if !response.status().is_success() {
        return Err(SapphireError::DownloadError( // Use specific error type
            cask.token.clone(),
            url_str.to_string(),
...
pub fn install_cask(
    ...
    if non_extensions.contains(&detected_extension.as_str()) {
        info!("Download path '{}' has no definite extension ('{}'), attempting content detection.", download_path.display(), detected_extension);
        // Use infer crate to detect type based on content
        match infer::get_from_path(download_path) { // Propagate potential IO errors? Use map_err
            Ok(Some(kind)) => {
            ...
            Ok(None) => {
                 error!("Could not determine file type from content for: {}", download_path.display());
                 return Err(SapphireError::Generic(format!(
            ...
            Err(e) => {
                 error!("Error reading file for type detection {}: {}", download_path.display(), e);
                 return Err(SapphireError::Io(e)); // Return IO error
            }
        }
    } else {
         debug!("Using file extension for type detection: {}", detected_extension);
    }
    ...
    match detected_extension.as_str() {
        ...
        "gz" | "bz2" | "xz" | "tar" => { // Handle tarballs potentially (if cask delivers them)
             // Pass the detected type! (e.g., "gz", "bz2", "xz", "tar")
             let archive_type_for_extraction = detected_extension.as_str();
             debug!("Extracting TAR archive ({}) {} to stage {}...", archive_type_for_extraction, download_path.display(), stage_path.display());
        ...
        _ => {
            error!("Unsupported container/installer type '{}' for staged installation derived from {}", detected_extension, download_path.display());
            return Err(SapphireError::Generic(format!(
    ...
    if let Some(artifacts_def) = &cask.artifacts { // Use the correct variable name
            ...
            if let Some(artifact_obj) = artifact_value.as_object() {
                ...
                if let Some((key, value)) = artifact_obj.iter().next() {
                    ...
                    let result: Result<Vec<InstalledArtifact>> = match key.as_str() {
                            ...
                            if let Some(app_names) = value.as_array() {
                                for app_name_val in app_names {
                                     if let Some(app_name) = app_name_val.as_str() {
                                        ...
                                        debug!("Attempting to install app artifact: {}", staged_app_path.display());
                                        match artifacts::app::install_app_from_staged(
                                            ...
                                            config,
                                             ...
                                             Ok(mut artifacts) => app_artifacts.append(&mut artifacts),
                                             Err(e) => {
                        ...
                        "pkg" => { // Handle PKG found *inside* DMG/ZIP
                             ...
                             if let Some(pkg_names) = value.as_array() {
                                 for pkg_val in pkg_names {
                                     if let Some(pkg_name) = pkg_val.as_str() {
                                         ...
                                         debug!("Attempting to install staged pkg artifact: {}", staged_pkg_path.display());
                                         match artifacts::pkg::install_pkg_from_path(cask, &staged_pkg_path, &cask_version_install_path, config) {
                                             Ok(mut artifacts) => installed_pkgs.append(&mut artifacts),
                                             Err(e) => {
                                        ...
                                        warn!("Non-string value found in 'pkg' artifact array: {:?}", pkg_val);
                                     }
                                 }
                             } else {
                        ...
                        "suite" => artifacts::suite::install_suite(cask, stage_path, &cask_version_install_path, config),
                        "installer" => artifacts::installer::run_installer(cask, stage_path, &cask_version_install_path, config),
                        "binary" => artifacts::binary::install_binary(cask, stage_path, &cask_version_install_path, config),
                        "manpage" => artifacts::manpage::install_manpage(cask, stage_path, &cask_version_install_path, config),
                        "colorpicker" => artifacts::colorpicker::install_colorpicker(cask, stage_path, &cask_version_install_path, config),
                        "dictionary" => artifacts::dictionary::install_dictionary(cask, stage_path, &cask_version_install_path, config),
                        "font" => artifacts::font::install_font(cask, stage_path, &cask_version_install_path, config),
                        "input_method" => artifacts::input_method::install_input_method(cask, stage_path, &cask_version_install_path, config),
                        "internet_plugin" => artifacts::internet_plugin::install_internet_plugin(cask, stage_path, &cask_version_install_path, config),
                        "keyboard_layout" => artifacts::keyboard_layout::install_keyboard_layout(cask, stage_path, &cask_version_install_path, config),
                        "prefpane" => artifacts::prefpane::install_prefpane(cask, stage_path, &cask_version_install_path, config),
                        "qlplugin" => artifacts::qlplugin::install_qlplugin(cask, stage_path, &cask_version_install_path, config),
                        "mdimporter" => artifacts::mdimporter::install_mdimporter(cask, stage_path, &cask_version_install_path, config),
                        "screen_saver" => artifacts::screen_saver::install_screen_saver(cask, stage_path, &cask_version_install_path, config),
                        "service" => artifacts::service::install_service(cask, stage_path, &cask_version_install_path, config),
                        "audio_unit_plugin" => artifacts::audio_unit_plugin::install_audio_unit_plugin(cask, stage_path, &cask_version_install_path, config),
                        "vst_plugin" => artifacts::vst_plugin::install_vst_plugin(cask, stage_path, &cask_version_install_path, config),
                        "vst3_plugin" => artifacts::vst3_plugin::install_vst3_plugin(cask, stage_path, &cask_version_install_path, config),
                        // Stanzas processed at install time but don't move files from stage
                        "zap" => artifacts::zap::install_zap(cask, config), // Zap doesn't use stage_path
                        "preflight" => artifacts::preflight::run_preflight(cask, stage_path, config), // Uses stage_path
                    ...
                    match result {
                        Ok(installed) => {
                            if !installed.is_empty() { // Only log if artifacts were actually added
                                debug!("Successfully processed artifact '{}', added {} items.", key, installed.len());
    ...
    if !artifact_install_errors.is_empty() {
         error!("Encountered {} errors installing artifacts for cask '{}'. Installation incomplete.", artifact_install_errors.len(), cask.token);
    ...
    let actual_install_count = all_installed_artifacts.iter().filter(|a| !matches!(a, InstalledArtifact::PkgUtilReceipt {..} | InstalledArtifact::Launchd {..})).count();
    ...
    if actual_install_count == 0 {
        ...
        write_cask_manifest(cask, &cask_version_install_path, all_installed_artifacts)?; // Write potentially empty installable manifest
    } else {
...
pub fn write_receipt(
    ...
    debug!("Successfully wrote legacy receipt with {} artifact entries.", artifacts.len());
    Ok(())
}
