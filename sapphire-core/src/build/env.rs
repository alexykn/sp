use crate::model::formula::FormulaDependencies; // Use trait for name() and install_prefix()
use crate::utils::error::{Result, SapphireError};
use crate::build::devtools; // Import the devtools module
use std::{
    collections::{HashMap, HashSet},
    // Removed unused: env, process::Command,
    path::{Path, PathBuf},
};
use log::{debug, info}; // Use logging

// Constants for filtering environment variables remain the same...
const ENV_VARS_TO_REMOVE: &[&str] = &[
    ...
    "HOMEBREW_DEBUG", "HOMEBREW_VERBOSE", "HOMEBREW_DEVELOPER",
    ...
    "HOMEBREW_INSTALL_BADGE", "HOMEBREW_LOGS", "HOMEBREW_MAKE_JOBS",
...
const ENV_VARS_TO_KEEP: &[&str] = &[
    "USER", "LOGNAME", "HOME", "TMPDIR", "TERM", "SHELL", "EDITOR",
    "LANG", "LC_ALL", "LC_CTYPE",
    "DISPLAY", "XAUTHORITY", "TZ",
];
...
impl BuildEnvironment {
    ...
    pub fn new<F: FormulaDependencies>(
        ...
        info!("Creating BuildEnvironment for formula '{}'...", formula.name());
        debug!("Sapphire Prefix: {}", sapphire_prefix.display());
        debug!("Provided installed dependency OPT paths: {:?}", all_installed_opt_paths);

        let mut vars = HashMap::new();
        let mut path_dirs = Vec::new();

        // 1. Filter initial environment
        filter_initial_environment(&mut vars);
        debug!("Initial environment filtering complete.");

        // 2. Resolve basic tools
        ...
        info!("Resolved tools: CC={}, CXX={}, SDK={}, macOS={}, ArchFlag='{}', InstallPrefix={}",
              cc.display(), cxx.display(), sdk_path.display(),
              macos_version, // Use the string directly
              arch_flag, formula_install_prefix.display());

        // --- Start building the environment ---

        // 3. Add paths from *all provided installed dependencies*
        ...
        for dep_opt_path in all_installed_opt_paths {
            ...
            let bin_path = dep_opt_path.join("bin"); if bin_path.is_dir() { path_dirs.push(bin_path); }
             let sbin_path = dep_opt_path.join("sbin"); if sbin_path.is_dir() { path_dirs.push(sbin_path); }
            let include_path = dep_opt_path.join("include"); if include_path.is_dir() { include_paths.push(include_path); }
            let lib_path = dep_opt_path.join("lib");
            if lib_path.is_dir() {
                lib_paths.push(lib_path.clone());
                let pkgconfig_lib_path = lib_path.join("pkgconfig"); if pkgconfig_lib_path.is_dir() { pkgconfig_paths.push(pkgconfig_lib_path); }
            }
             let share_path = dep_opt_path.join("share");
             if share_path.is_dir() {
                 let pkgconfig_share_path = share_path.join("pkgconfig"); if pkgconfig_share_path.is_dir() { pkgconfig_paths.push(pkgconfig_share_path); }
                 let aclocal_share_path = share_path.join("aclocal"); if aclocal_share_path.is_dir() { aclocal_paths.push(aclocal_share_path); }
             }
              let framework_path = dep_opt_path.join("Frameworks"); if framework_path.is_dir() { cmake_framework_paths.push(framework_path); }
              if dep_opt_path.is_dir() { cmake_prefix_paths.push(dep_opt_path.clone()); }
        }
         debug!("Dependency paths collected.");

        // 4. Add Sapphire's own bin/sbin paths
        let sapphire_bin = sapphire_prefix.join("bin"); if sapphire_bin.is_dir() { path_dirs.insert(0, sapphire_bin); }
        let sapphire_sbin = sapphire_prefix.join("sbin"); if sapphire_sbin.is_dir() { path_dirs.insert(0, sapphire_sbin); }
         debug!("Prepended Sapphire bin/sbin to PATH list.");

        // 5. Add system paths and compiler path
        if let Some(compiler_bin) = cc.parent() { path_dirs.insert(0, compiler_bin.to_path_buf()); debug!("Prepended compiler bin to PATH list: {}", compiler_bin.display()); }
        let standard_paths = ["/usr/bin", "/bin", "/usr/sbin", "/sbin"];
        for spath in standard_paths.iter().map(PathBuf::from) { if !path_dirs.iter().any(|p| p == &spath || p.starts_with(&spath)) { path_dirs.push(spath); } }

        // Deduplicate PATH
        let mut unique_path_dirs = Vec::new(); let mut seen_paths = HashSet::new();
        for dir in path_dirs { if seen_paths.insert(dir.clone()) { unique_path_dirs.push(dir); } }
        path_dirs = unique_path_dirs;

        let final_path_string = std::env::join_paths(path_dirs.iter()) // Use std::env::join_paths
        ...
        // 6. Set SDK and Deployment Target (macOS only)
        if cfg!(target_os = "macos") {
            if sdk_path != PathBuf::from("/") { vars.insert("SDKROOT".to_string(), sdk_path.to_string_lossy().to_string()); }
            vars.insert("MACOSX_DEPLOYMENT_TARGET".to_string(), macos_version.clone());
            debug!("Set SDKROOT={} MACOSX_DEPLOYMENT_TARGET={}", sdk_path.display(), macos_version);
        }

        // 7. Set Compiler Variables
        vars.insert("CC".to_string(), cc.to_string_lossy().to_string());
        vars.insert("CXX".to_string(), cxx.to_string_lossy().to_string());
        let stdlib_flag = if cfg!(target_os = "macos") { "-stdlib=libc++" } else { "" };
         debug!("Set CC={} CXX={}", cc.display(), cxx.display());
         if !stdlib_flag.is_empty() { debug!("Adding default C++ stdlib flag: {}", stdlib_flag); }

        // 8. Set Flags
        let cppflags = include_paths.iter().map(|p| format!("-I{}", p.display())).collect::<Vec<_>>().join(" ");
        vars.insert("CPPFLAGS".to_string(), cppflags.clone()); debug!("Set CPPFLAGS={}", cppflags);

        let sysroot_flag = if cfg!(target_os = "macos") && sdk_path != PathBuf::from("/") { format!("-isysroot {}", sdk_path.display()) } else { String::new() };
        let cflags = format!("{} -O2 {}", arch_flag, sysroot_flag).trim().to_string();
        vars.insert("CFLAGS".to_string(), cflags.clone()); debug!("Set CFLAGS={}", cflags);

        let cxxflags = format!("{} {}", cflags, stdlib_flag).trim().to_string();
        vars.insert("CXXFLAGS".to_string(), cxxflags.clone()); debug!("Set CXXFLAGS={}", cxxflags);

        let ldflags_lib_part = lib_paths.iter().map(|p| format!("-L{}", p.display())).collect::<Vec<_>>().join(" ");
        let ldflags = format!("{} {} {}", ldflags_lib_part, arch_flag, sysroot_flag).trim().to_string();
        vars.insert("LDFLAGS".to_string(), ldflags.clone()); debug!("Set LDFLAGS={}", ldflags);

        // 9. Set Build Parallelism
        let jobs = num_cpus::get().to_string();
        vars.insert("MAKEFLAGS".to_string(), format!("-j{}", jobs)); debug!("Set MAKEFLAGS=-j{}", jobs);

        // 10. Set PkgConfig Paths
    ...
    fn is_controlled_homebrew_var(key: &str) -> bool {
        matches!(key, "HOMEBREW_CC" | "HOMEBREW_CXX" | "HOMEBREW_CFLAGS" | "HOMEBREW_CXXFLAGS" |
    ...
    fn set_path_list_var(vars: &mut HashMap<String, String>, name: &str, paths: &[PathBuf]) -> Result<()> {
        ...
        if !existing_paths.is_empty() {
            ...
            for path in existing_paths { if seen.insert(path.clone()) { unique_paths.push(path); } }
            let joined_path = unique_paths.join(":");
             ...
             } else { debug!("No valid directories found for {}, not setting variable.", name); }
        } else { debug!("No existing directories provided for {}, not setting variable.", name); }
        Ok(())
    }
    ...
    pub fn apply_to_command(&self, command: &mut std::process::Command) {
        ...
        debug!("Applying sanitized environment to command: {:?}", command.get_program());
...
fn filter_initial_environment(vars: &mut HashMap<String, String>) {
    let initial_env: HashMap<String, String> = std::env::vars().collect(); // Use std::env directly
    ...
    *vars = HashMap::new(); // Clear the map first

    for (key, value) in initial_env.iter() {
        let key_upper = key.to_uppercase();
        if vars_to_remove_set.contains(key.as_str()) { debug!("Removing env var: {}", key); continue; }
        if key_upper.starts_with("HOMEBREW_") && !BuildEnvironment::is_controlled_homebrew_var(key) { debug!("Removing potentially interfering Homebrew env var: {}", key); continue; }
        if vars_to_keep_set.contains(key.as_str()) { debug!("Keeping env var: {}", key); vars.insert(key.clone(), value.clone()); }
        // Ignore others
    }
}