...
use crate::fetch::oci;
use crate::model::formula::{BottleFileSpec, Formula, FormulaDependencies}; // Added FormulaDependencies
...
fn get_bottle_for_platform(formula: &Formula) -> Result<(String, &BottleFileSpec)> {
    ...
    // Check for OS name fallback for non-ARM architectures
        ...
        if os_name != current_platform && stable_spec.files.contains_key(os_name) {
            // Ensure os_name is different and exists
            if let Some(spec) = stable_spec.files.get(os_name) {
                warn!(
...
async fn download_direct_url(client: &Client, url: &str, destination_path: &Path) -> Result<()> {
    ...
    if !response.status().is_success() {
        ...
        error!(
            "Direct download failed for {}: Status {} - {}",
    ...
    if temp_path.exists() {
        if let Err(e) = fs::remove_file(&temp_path) {
            warn!(
                "Could not remove existing temp file {}: {}",
                temp_path.display(),
                e
            );
        }
...

    {
        ...
        copy(&mut content.as_ref(), &mut dest_file).map_err(|e| {
            SapphireError::Io(std::io::Error::new(
                e.kind(),
                format!("Failed to write download to {}: {}", temp_path.display(), e),
            ))
        })?;
...
pub fn verify_bottle_checksum(bottle_path: &Path, expected_sha256: &str) -> Result<()> {
    if expected_sha256.is_empty() {
        warn!(
            "Skipping checksum verification for {} - no expected checksum provided.",
            bottle_path.display()
        );
        return Ok(());
    }
    info!("Verifying checksum for bottle: {}", bottle_path.display());
    let mut file = File::open(bottle_path).map_err(|e| {
        SapphireError::Io(std::io::Error::new(
            e.kind(),
            format!(
    ...
    if !actual_sha256.eq_ignore_ascii_case(expected_sha256) {
        ...
        return Err(SapphireError::ChecksumError(format!(
                ...
                match fs::read_to_string(path) {
                    Ok(content) => {
                        ...
                        if content.starts_with("#!") {
                            ...
                            for (placeholder, replacement) in &replacements {
                                if placeholder.starts_with("@@HOMEBREW_")
                                    && current_line_content.contains(placeholder)
                                {
                                    ...
                                    current_line_content =
                                        current_line_content.replace(placeholder, replacement);
                                    debug!(
                                        "  Relocated SHEBANG placeholder '{}' in: {}",
                                        placeholder,
                                        path.display()
                                    );
                        ...
                        for (placeholder, replacement) in &replacements {
                            ...
                            let search_area = if modified && content.starts_with("#!") {
                                let first_line_end_pos = modified_content
                                    .find('\n')
                            ...
                            if search_area.contains(placeholder) {
                                ...
                                modified_content =
                                    modified_content.replace(placeholder, replacement);
                                if !modified {
                                    // Only log first modification type if shebang wasn't modified
                                    debug!(
                                        "  Relocated standard placeholder '{}' in: {}",
                                        placeholder,
                                        path.display()
                                    );
                                }
                                modified = modified || (modified_content.len() != original_len);
                    ...
                    Err(e) => {
                        debug!(
                            "  Skipping relocation for {} (read failed): {}",
                            path.display(),
                            e
                        );
                    }
