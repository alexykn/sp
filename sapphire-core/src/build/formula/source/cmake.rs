...
pub fn cmake_build(
    ...
    let cmake_exe =
        which::which_in("cmake", build_env.get_path_string(), Path::new(".")).map_err(|_| {
    ...
    cmd.arg("install").current_dir(&build_subdir);
    build_env.apply_to_command(&mut cmd);
    let output = cmd.output().map_err(|e| {
        SapphireError::CommandExecError(format!("Failed to execute make install (CMake): {}", e))
    })?;

    if !output.status.success() {
        println!("CMake make install failed with status: {}", output.status);
        eprintln!(
            "CMake make install stdout:\n{}",
            String::from_utf8_lossy(&output.stdout)
        );
        eprintln!(
            "CMake make install stderr:\n{}",
            String::from_utf8_lossy(&output.stderr)
        );
        return Err(SapphireError::Generic(format!(
            "CMake make install failed with status: {}",
            output.status
        )));
    } else {
