...
pub fn cmake_build(
    ...
    let cmake_exe =
        which::which_in("cmake", build_env.get_path_string(), Path::new(".")) // Check CWD and PATH
            .map_err(|_| {
    ...
    let ninja_exe =
        which::which_in("ninja", build_env.get_path_string(), Path::new(".")) // Find ninja
            .map_err(|_| {
                SapphireError::BuildEnvError(
    ...
    if !output_install.status.success() {
        println!("Ninja install failed with status: {}", output_install.status);
        eprintln!(
