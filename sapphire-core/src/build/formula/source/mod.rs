...
fn determine_archive_type(archive_path: &Path, context: &str) -> Result<&'static str> {
    match infer::get_from_path(archive_path)? { // Handles infer's IO error
        Some(kind) => {
            ...
            if SUPPORTED_ARCHIVE_EXTENSIONS.contains(&ext) {
                ...
                error!("Unsupported inferred archive type '{}' for {}: {}", ext, context, archive_path.display());
                Err(SapphireError::Generic(format!(
                    "Unsupported inferred archive type '{}' for {}: {}",
                    ext, context, archive_path.display()
                )))
            }
        }
        None => {
            // Fallback to extension if infer fails
            let fallback_ext = archive_path.extension().and_then(|s| s.to_str()).unwrap_or("");
            warn!(
                "Could not infer archive type for {}, falling back to extension '{}'.",
                archive_path.display(), fallback_ext
            );
            if fallback_ext.is_empty() {
                error!("Cannot determine archive type for {}: {}", context, archive_path.display());
                return Err(SapphireError::Generic(format!("Cannot determine archive type for {}: {}", context, archive_path.display())));
            }
            ...
            if SUPPORTED_ARCHIVE_EXTENSIONS.contains(&fallback_ext) {
                 ...
                 SUPPORTED_ARCHIVE_EXTENSIONS.iter()
                     .find(|&&s| s == fallback_ext)
                     .copied()
                     .ok_or_else(|| SapphireError::Generic(format!("Internal error: Matched extension '{}' not found in static list", fallback_ext))) // Should not happen
            } else {
                error!("Unsupported fallback archive type '{}' for {}: {}", fallback_ext, context, archive_path.display());
                Err(SapphireError::Generic(format!("Unsupported fallback archive type '{}' for {}: {}", fallback_ext, context, archive_path.display())))
            }
        }
    }
}
...
pub async fn build_from_source(
    ...
    let temp_build_dir = tempfile::Builder::new()
        ...
        .map_err(|e| SapphireError::IoError(format!("Failed create temp build dir in {}: {}", temp_dir_base.display(), e)))?;
    ...
    if !resources.is_empty() {
        ...
        for (res_name, resource_archive_path) in download_results {
            ...
            extract_archive(&resource_archive_path, &stage_path, 0, resource_archive_type_str)?; // strip_components = 0
...
fn detect_and_build(
        ...
        .filter_map(|e| e.ok()) // Filter out errors during walk
    {
        ...
        if current_path.is_file() {
            ...
            if let Some((marker, _system_name, requires_root)) = markers.iter().find(|(m, _, _)| *m == file_name) {

                // Skip non-root markers if they require root detection
                if *requires_root && current_depth > 0 {
                    debug!("Skipping marker '{}' found at depth {}, requires root.", marker, current_depth);
                    continue;
                }
                ...
                let mut score = match current_depth {
                    ...
                    1 | 2 => { // Nested matches (only relevant for CMake/Meson now)
                        if *marker == "CMakeLists.txt" || *marker == "meson.build" {
                             let parent_dir_name = parent_dir.file_name().map(|f| f.to_os_string());
                             if parent_dir_name.map_or(false, |name| preferred_subdirs.contains(&name)) {
                                2 // Preferred subdirectory
                             } else {
                                1 // Other subdirectory
                             }
                        } else {
                            // Other markers shouldn't be scored if nested due to requires_root check above
                ...
                let is_better = match best_match {
                    ...
                    Some((ref existing_marker, _, _, existing_score)) if score == existing_score => {
                         let existing_priority = markers.iter().position(|(m, _, _)| m == existing_marker).unwrap_or(usize::MAX);
                ...
                if is_better {
                     debug!(
                        ...
                        marker, parent_dir.display(), current_depth, score, current_priority
                     );
                     best_match = Some((
                        marker.to_string(),
    ...
    if let Some((marker_name, marker_dir, depth, score)) = best_match {
        ...
        let source_path_for_build = if depth > 0 && (marker_name == "CMakeLists.txt" || marker_name == "meson.build") {
            info!("Using nested marker directory for build: {}", marker_dir.display());
            marker_dir.as_path()
        } else {
            // Default to the root build directory for root markers or other (unexpected) nested ones.
            info!("Using root build directory for build: {}", build_dir.display());
            build_dir
        };
...
fn dispatch_build(
    ...
    install_dir: &Path,
    build_env: &BuildEnvironment,
    _all_installed_paths: &[PathBuf], // Keep in case Go needs it, though it's handled separately now
) -> Result<()> {
    // Remember: The *current working directory* for dispatch_build is still the root `build_dir`.
    // The build functions themselves might change CWD or operate relative to `source_dir_for_build`.
    match marker_filename {
        ...
        "Makefile.PL" | "Configure" => {
            ...
            // It might internally use source_dir_for_build if needed, but typically runs `perl Makefile.PL` in CWD
...
fn install_resource(
    ...
    if stage_path.join("Makefile.PL").exists() {
        ...
        // We could potentially add more resource build system detections here (e.g., simple make install)
        warn!(
...
fn install_perl_resource(
    ...
    let perl_exe = which::which_in("perl", build_env.get_path_string(), Path::new("."))
        .map_err(|_| SapphireError::BuildEnvError("perl not found in build env PATH for resource install".to_string()))?;
    let make_exe = which::which_in("make", build_env.get_path_string(), Path::new("."))
         .map_err(|_| SapphireError::BuildEnvError("make not found in build env PATH for resource install".to_string()))?;
    ...
    configure_cmd.arg("Makefile.PL")
                 .arg(format!("INSTALL_BASE={}", libexec_path.display()));
    configure_cmd.env_clear().envs(&cmd_env); // Apply full env
    run_command(&mut configure_cmd, &format!("Perl Makefile.PL for resource '{}'", resource.name))?;

    // Run make
    let mut make_cmd = Command::new(make_exe.clone());
    make_cmd.env_clear().envs(&cmd_env); // Apply full env
    run_command(&mut make_cmd, &format!("make for Perl resource '{}'", resource.name))?;

    // Run make install
    let mut install_cmd = Command::new(make_exe);
    install_cmd.arg("install");
    install_cmd.env_clear().envs(&cmd_env); // Apply full env
    run_command(&mut install_cmd, &format!("make install for Perl resource '{}'", resource.name))?;

    info!("   -> Successfully installed Perl resource '{}'", resource.name);
    Ok(())
}
...
fn install_python_resource(
    ...
    let python_exe = which::which_in("python3", build_env.get_path_string(), Path::new("."))
        ...
        .map_err(|_| SapphireError::BuildEnvError("python not found in build env PATH for resource install".to_string()))?;

    // Determine Python version for site-packages path
    let mut version_cmd = Command::new(&python_exe);
    version_cmd.args(["-c", "import sys; print(f'{sys.version_info.major}.{sys.version_info.minor}')"]);
    ...
    if python_version_str.is_empty() {
        return Err(SapphireError::BuildEnvError("Could not determine python version for resource path.".to_string()));
    }
    ...
    let new_pythonpath = if current_pythonpath.is_empty() {
        pythonpath_entry
         ...
         // Prepend target dir
        format!("{}:{}", pythonpath_entry, std::ffi::OsStr::new(&current_pythonpath).to_string_lossy())
    };
    ...
    install_cmd.arg("setup.py")
               .arg("install")
               .arg(format!("--prefix={}", libexec_path.join("vendor").display())); // Install under vendor prefix
    install_cmd.env_clear().envs(&cmd_env); // Apply full env
    run_command(&mut install_cmd, &format!("Python setup.py install for resource '{}'", resource.name))?;
