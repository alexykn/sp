...
fn build_api_client(config: &Config) -> Result<Client> {
    ...
    if let Some(token) = &config.github_api_token {
        ...
        match format!("Bearer {}", token).parse() {
             ...
             Err(e) => { error!("Failed to parse GitHub API token into header value: {}", e); }
        }
...
pub async fn fetch_raw_formulae_json(endpoint: &str) -> Result<String> {
    ...
    if !response.status().is_success() {
        ...
        error!("HTTP request to {} returned non-success status: {}. Body: {}", url, status, body);
        return Err(SapphireError::Api(format!("HTTP status {} from {}. Response body: {}", status, url, body)));
    }
    ...
    if body.trim().is_empty() {
        ...
        return Err(SapphireError::Api(format!("Empty response body received from {}", url)));
    }
    Ok(body)
}
...
pub async fn fetch_formula(name: &str) -> Result<serde_json::Value> {
    ...
    if let Ok(body) = direct_fetch_result {
        ...
        warn!("Direct fetch for formula '{}' failed. Fetching full list as fallback.", name);
        ...
        Err(SapphireError::NotFound(format!("Formula '{}' not found in API list", name)))
    }
}
...
pub async fn fetch_cask(token: &str) -> Result<serde_json::Value> {
    ...
    if let Ok(body) = direct_fetch_result {
        ...
        return Ok(cask);
         ...
         warn!("Direct fetch for cask '{}' failed. Fetching full list as fallback.", token);
         ...
         Err(SapphireError::NotFound(format!("Cask '{}' not found in API list", token)))
     }
}
...
async fn fetch_github_api_json(endpoint: &str, config: &Config) -> Result<Value> {
    ...
    if !response.status().is_success() {
        let status = response.status();
        let body = response.text().await.unwrap_or_else(|e| format!("(Failed to read response body: {})", e));
        error!("GitHub API request to {} returned non-success status: {}. Body: {}", url, status, body);
        return Err(SapphireError::Api(format!("HTTP status {} from {}. Response body: {}", status, url, body)));
    }
...
pub async fn get_formula(name: &str) -> Result<Formula> {
    ...
    info!("Fetching and parsing formula data for '{}' from {}", name, url);
    ...
    match response_result {
        Ok(response) => {
            ...
            if !status.is_success() {
                 error!("Failed to fetch formula {} (Status {}): {}", name, status, text);
                 return Err(SapphireError::Api(format!("Failed to fetch formula {}: Status {}", name, status)));
            }
             if text.trim().is_empty() {
                 error!("Received empty body when fetching formula {}", name);
                 return Err(SapphireError::Api(format!("Empty response body for formula {}", name)));
             }
                  ...
                  Err(_) => {
                      ...
                      match serde_json::from_str::<Vec<Formula>>(&text) {
                           Ok(mut formulas) if !formulas.is_empty() => {
                               info!("Parsed formula {} from a single-element array response.", name);
                               Ok(formulas.remove(0))
                           },
                           Ok(_) => { // Empty array
                                error!("Received empty array when fetching formula {}", name);
                                Err(SapphireError::NotFound(format!("Formula '{}' not found (empty array returned)", name)))
                           },
                           Err(e_vec) => { // Failed to parse as array too
...
pub async fn get_all_casks() -> Result<CaskList> {
     ...
     let casks: Vec<Cask> = serde_json::from_str(&raw_data)
         .map_err(|e| {
