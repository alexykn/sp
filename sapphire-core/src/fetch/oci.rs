...
pub async fn download_oci_blob(
    ...
    let tmp = destination_path.with_file_name(
        format!(".{}.download", destination_path.file_name().unwrap().to_string_lossy())
    );
...
async fn determine_auth(
    ...
    if let Some(basic) = &config.docker_registry_basic_auth {
        ...
        return Ok(OciAuth::Basic { encoded: basic.clone() });
    }

    if registry_domain.eq_ignore_ascii_case(DEFAULT_GHCR_DOMAIN) && !repo_path.is_empty() {
        debug!("Anonymous token fetch for {} scope={}", registry_domain, repo_path);
...
async fn fetch_anonymous_token(
    ...
    for attempt in 0..=MAX_RETRIES {
        debug!("Token attempt {}/{} from {}", attempt+1, MAX_RETRIES+1, token_url);

        match client.get(&token_url).send().await {
            Ok(resp) if resp.status().is_success() => {
                let tok: OciTokenResponse = resp.json().await
                    .map_err(|e| SapphireError::ApiRequestError(format!("Parse token response: {}", e)))?;
                return Ok(tok.token);
            }
            Ok(resp) => {
...
async fn execute_oci_request(
    ...
    match auth {
        OciAuth::AnonymousBearer { token }
        | OciAuth::ExplicitBearer   { token } if !token.is_empty() =>
