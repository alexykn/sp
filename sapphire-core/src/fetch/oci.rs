...
async fn fetch_oci_resource<T: serde::de::DeserializeOwned>(
    ...
    let response =
        execute_oci_request(config, client, resource_url, accept_header, &auth, 1).await?; // Allow 1 retry on 401

    let response_text = response.text().await.map_err(|e| SapphireError::Http(e))?;

    debug!(
        "OCI Resource Response Body ({}): [Body Length: {}]",
        resource_url,
        response_text.len()
    );
...
pub async fn download_oci_blob(
    ...
    // Pass config to execute_oci_request
    let response =
        execute_oci_request(config, client, blob_url, OCI_LAYER_V1_TYPE, &auth, 1).await?; // Allow 1 retry
    ...
    debug!(
        "Downloading blob to temporary path: {}",
        temp_path.display()
    );
    if temp_path.exists() {
        if let Err(e) = std::fs::remove_file(&temp_path) {
            warn!(
                "Could not remove existing temporary blob file {}: {}",
                temp_path.display(),
                e
            );
        }
...

    {
        ...
        let mut dest_file = File::create(&temp_path).map_err(|e| {
            SapphireError::Io(std::io::Error::new(
                e.kind(),
                format!(
                    "Failed to create temp blob file {}: {}",
                    temp_path.display(),
                    e
                ),
            ))
    ...
    std::fs::rename(&temp_path, destination_path).map_err(|e| {
        SapphireError::Io(std::io::Error::new(
            e.kind(),
            format!(
                "Failed to move temp blob file {} to {}: {}",
                temp_path.display(),
...
async fn fetch_anonymous_token(
    ...
    if !response.status().is_success() {
        ...
        error!(
            "Failed to fetch anonymous token: HTTP Status {} - {}",
            status, body
        );
    ...
    match response.json::<OciTokenResponse>().await {
        ...
        Err(e) => {
            ...
            Err(SapphireError::ApiRequestError(format!(
                "Failed to parse OCI token response: {}",
                e
            )))
        }
    }
...
async fn execute_oci_request(
    ...
    if status.is_success() {
        ...
        && matches!(auth, OciAuth::AnonymousBearer { .. })
        && retries_left > 0
    {
        ...
        let refreshed_auth = match determine_auth(config, client, registry_domain, repo_path).await
        {
            ...
            Err(e) => {
                warn!(
                    "Error during token refresh attempt: {}. Retrying with original auth details.",
                    e
                );
