...
pub enum PipelineJobResult {
    ...
    InstallErr(String, PackageType, SpError),
    UpgradeErr(String, PackageType, String, SpError), // Include old version
    ReinstallErr(String, PackageType, SpError),
}
...
type PlanResult = Result<(Vec<PipelineJob>, Vec<(String, SpError)>, HashSet<String>)>;
...
impl PipelineExecutor {
    ...
    pub async fn execute_pipeline(
        ...
        if let Err(e) = pump_handle.await {
            ...
            overall_errors.push((
                ...
                SpError::Generic(format!("Worker coordination failed: {e}")),
            ));
        }
    ...
    async fn plan_package_operations(
        ...
        let mut errors: Vec<(String, SpError)> = Vec::new();
        ...
        match command_type {
            CommandType::Install => {
                ...
                for name in initial_targets {
                    ...
                    match sp_core::installed::get_installed_package(name, config).await? {
                ...
                for name in initial_targets {
                    ...
                    match sp_core::installed::get_installed_package(name, config).await? {
                        ...
                        None => {
                            ...
                            error!("✖ {msg}");
                            errors.push((name.clone(), SpError::NotFound(msg)));
                            processed.insert(name.clone());
                        }
                    }
                }
            ...
            CommandType::Upgrade { all } => {
                ...
                let packages_to_check = if all {
                    sp_core::installed::get_installed_packages(config).await?
                } else {
            ...
            match opt_def {
                ...
                None => {
                    ...
                    if !errors.iter().any(|(n, _)| n == name) {
                        ...
                        error!("✖ {msg}");
                        errors.push((name.clone(), SpError::Generic(msg)));
                    }
                    processed.insert(name.clone());
                }
            }
                    ...
                    if !formulae_for_resolution.contains_key(formula_dep) {
                            ...
                            .remove(formula_dep)
                        {
                            ...
                            _ => {
                                ...
                                if !errors.iter().any(|(n, _)| n == formula_dep) {
                                    errors.push((formula_dep.clone(), SpError::NotFound(msg)));
                                }
                            }
                        }
                    }
            ...
            match resolver.resolve_targets(&resolution_target_names) {
                ...
                Err(e) => {
                    ...
                    for name in resolution_target_names {
                        if !errors.iter().any(|(n, _)| n == &name) {
                            errors.push((name.clone(), SpError::DependencyError(e.to_string())));
                        }
                    }
        ...
        if let Some(graph) = resolved_formula_graph {
            ...
            for (token, cask_arc) in cask_deps_map {
                ...
                if !jobs.iter().any(|j| match &j.target {
                    ...
                    if sp_core::installed::get_installed_package(&token, config)
                        .await?
                        .is_none()
                    {
                        jobs.push(PipelineJob {
    ...
    fn collect_results(result_rx: Receiver<PipelineJobResult>) -> Vec<(String, SpError)> {
        let mut install_errors: Vec<(String, SpError)> = Vec::new();
...
async fn download_target_file(
    ...
    .map_err(|e| {
        ...
        if matches!(e, SpError::DownloadError(_, _, _)) {
            e
        } else {
            SpError::DownloadError(
