...
use sp_core::build;
use sp_core::build::cask::{CaskInstallManifest, InstalledArtifact};
...
impl Uninstall {
    ...
    pub async fn run(&self, config: &Config, cache: Arc<Cache>) -> Result<()> {
        ...
        let mut errors: Vec<(String, SpError)> = Vec::new(); // Store errors per package name
        ...
        for name in names {
            ...
            if name.contains('/') || name.contains("..") {
                ...
                errors.push((
                    name.to_string(),
                    SpError::Generic("Invalid package name".into()),
                ));
                continue;
            }
            ...
            if is_formula {
                ...
                if formula.name().contains('/') || formula.name().contains("..") {
                    ...
                    errors.push((
                        name.to_string(),
                        SpError::Generic("Invalid formula name".into()),
                    ));
                    pb.finish_and_clear();
                    continue;
                }
                ...
                let mut unlink_error: Option<SpError> = None;
                match build::formula::link::unlink_formula_artifacts(&formula, config) {
                    Ok(_) => {
                ...
                if let Err(e) = fs::remove_dir_all(&cellar_path) {
                    let removal_error = SpError::Io(Arc::new(std::io::Error::new(
                        e.kind(),
            ...
            match api::fetch_cask(name).await {
                ...
                Ok(cask_json) => {
                    let cask: Cask = match serde_json::from_value(cask_json) {
                        ...
                        Err(e) => {
                            ...
                            errors.push((name.to_string(), SpError::Json(Arc::new(e))));
                            pb.finish_and_clear();
                            continue;
                        }
                    };
                    ...
                    if cask.token.contains('/') || cask.token.contains("..") {
                        ...
                        errors.push((
                            name.to_string(),
                            SpError::Generic("Invalid cask token".into()),
                        ));
                        pb.finish_and_clear();
                        continue;
                    }
                    ...
                    let installed_version = match cask.installed_version(config) {
                        ...
                        None => {
                            ...
                            if errors
                                .iter()
                                .all(|(n, e)| n != name || !matches!(e, SpError::NotFound(_)))
                            {
                                // Only add error if it wasn't already marked as not found
                                errors.push((
                                    name.to_string(),
                                    SpError::NotFound(format!("Cask '{name}' is not installed")),
                                ));
                            }
                            pb.finish_and_clear();
                            continue;
                    ...
                    if let Err(e) = fs::remove_dir_all(&cask_version_path) {
                        ...
                        errors.push((
                            name.to_string(),
                            SpError::Io(Arc::new(std::io::Error::new(
                                e.kind(),
                ...
                Err(SpError::NotFound(_)) => {
                        ...
                        .all(|(n, e)| n != name || !matches!(e, SpError::NotFound(_)))
                    {
                        errors.push((
                            name.to_string(),
                            SpError::NotFound(format!("Formula or Cask '{name}' not found")),
                        ));
                    }
                    pb.finish_and_clear();
                    continue;
