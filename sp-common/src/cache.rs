...
impl Cache {
    ...
    pub fn load_raw(&self, filename: &str) -> Result<String> {
        ...
        if !path.exists() {
            return Err(SpError::Cache(format!(
                "Cache file {filename} does not exist"
            )));
        }

        fs::read_to_string(&path).map_err(|e| SpError::Cache(format!("IO error: {e}")))
    }
    ...
    pub fn is_cache_valid(&self, filename: &str) -> Result<bool> {
        ...
        let age = SystemTime::now()
            ...
            .map_err(|e| SpError::Cache(format!("System time error: {e}")))?;
...
pub fn get_cache_dir() -> Result<PathBuf> {
    let base_cache_dir = dirs::cache_dir()
        .ok_or_else(|| SpError::Cache("Could not determine system cache directory".to_string()))?;
...
pub fn load_from_cache<T: DeserializeOwned>(filename: &str) -> Result<T> {
    ...
    if !path.exists() {
        ...
        return Err(SpError::Cache("Cache file does not exist".to_string()));
    }
