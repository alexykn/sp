// FILE: sp-core/src/dependency/resolver.rs
...
pub struct DependencyResolver<'a> {
    ...
    // Store Arc<SpError> instead of SpError
    errors: HashMap<String, Arc<SpError>>,
}
...
impl<'a> DependencyResolver<'a> {
    ...
    pub fn resolve_targets(&mut self, targets: &[String]) -> Result<ResolvedGraph> {
        ...
        let sorted_list = match self.topological_sort() {
            Ok(list) => list,
            Err(e @ SpError::DependencyError(_)) => {
    ...
    fn resolve_recursive(
        ...
        if self.visiting.contains(name) {
            ...
            return Err(SpError::DependencyError(format!(
                ...
                None => {
                    ...
                    match self.context.formulary.load_formula(name) {
                        ...
                        Err(e) => {
                            ...
                            self.errors
                                .insert(name.to_string(), Arc::new(SpError::NotFound(msg)));
        ...
        for dep in dep_snapshot.formula.dependencies()? {
            ...
            if let Err(e) = self.resolve_recursive(dep_name, dep_tags, false) {
                ...
                let is_cycle = matches!(e, SpError::DependencyError(_));
                let msg = e.to_string();
                ...
                if is_cycle {
                    self.visiting.remove(name);
                    return Err(SpError::DependencyError(
