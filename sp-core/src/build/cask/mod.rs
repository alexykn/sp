...
pub async fn download_cask(cask: &Cask, cache: &Cache) -> Result<PathBuf> {
    let url_field = cask
        .url
        .as_ref()
        .ok_or_else(|| SpError::Generic(format!("Cask {} has no URL", cask.token)))?;
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|e| SpError::Http(std::sync::Arc::new(e)))?;
    if !response.status().is_success() {
        return Err(SpError::DownloadError(
            cask.token.clone(),
            url_str.to_string(),
...
pub fn write_receipt(
    ...
    let timestamp = SystemTime::now()
        ...
        .map_err(|e: SystemTimeError| SpError::Generic(format!("System time error: {e}")))?
        .as_secs();
    ...
    if let Some(parent) = receipt_path.parent() {
        fs::create_dir_all(parent).map_err(|e| SpError::Io(std::sync::Arc::new(e)))?;
    }
    let mut file =
        fs::File::create(&receipt_path).map_err(|e| SpError::Io(std::sync::Arc::new(e)))?;
    serde_json::to_writer_pretty(&mut file, &receipt_data)
        .map_err(|e| SpError::Json(std::sync::Arc::new(e)))?;
    debug!(
...
pub fn write_cask_manifest(
    ...
    serde_json::to_writer_pretty(writer, &manifest_data).map_err(|e| {
        ...
        SpError::Json(std::sync::Arc::new(e))
    })?;
    debug!(
