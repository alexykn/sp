// **File:** sp-core/src/build/devtools.rs (New file)
use std::env;
...
pub fn find_compiler(name: &str) -> Result<PathBuf> {
    ...
    which::which(name).map_err(|e| {
        SpError::BuildEnvError(format!("Failed to find compiler '{name}' on PATH: {e}"))
    })
}
    ...
    if cfg!(target_os = "macos") {
        ...
        match output {
            Ok(out) if out.status.success() => {
                ...
                if !sdk_path.exists() {
                    return Err(SpError::BuildEnvError(format!(
            ...
            Ok(out) => {
                // xcrun ran but failed
                let stderr = String::from_utf8_lossy(&out.stderr);
                Err(SpError::BuildEnvError(format!(
