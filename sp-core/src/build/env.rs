...
pub struct BuildEnvironment {
    ...
    /// The root installation directory for sp (e.g., /opt/homebrew or /usr/local).
    #[allow(dead_code)]
    sp_prefix: PathBuf,
...
impl BuildEnvironment {
    ...
    pub fn new<F: FormulaDependencies>(
        ...
        debug!("sp Prefix: {}", sp_prefix.display());
        debug!(
        ...
        let sp_bin = sp_prefix.join("bin");
        if sp_bin.is_dir() {
            path_dirs.insert(0, sp_bin);
        }
        let sp_sbin = sp_prefix.join("sbin");
        if sp_sbin.is_dir() {
            path_dirs.insert(0, sp_sbin);
        }
        debug!("Prepended sp bin/sbin to PATH list.");
        ...
        let final_path_string = std::env::join_paths(path_dirs.iter())
            .map_err(|e| SpError::BuildEnvError(format!("Failed to join PATH: {e}")))?
            .into_string()
            .map_err(|os_str| {
                SpError::BuildEnvError(format!(
        ...
        Ok(Self {
            ...
            sp_prefix: sp_prefix.to_path_buf(),
