// FILE: sp-core/src/build/formula/source/cmake.rs
...
pub fn cmake_build(
    ...
    let cmake_build_subdir_name = "sp-cmake-build";
    let cmake_build_dir = build_dir.join(cmake_build_subdir_name);
    fs::create_dir_all(&cmake_build_dir).map_err(|e| SpError::Io(std::sync::Arc::new(e)))?;

    let cmake_exe =
        which::which_in("cmake", build_env.get_path_string(), build_dir).map_err(|_| {
            SpError::BuildEnvError("cmake command not found in build environment PATH.".to_string())
        })?;
    ...
    let ninja_exe = which::which_in("ninja", build_env.get_path_string(), &cmake_build_dir)
        .map_err(|_| {
            SpError::BuildEnvError("ninja command not found in build environment PATH.".to_string())
        })?;
