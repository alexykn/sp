// FILE: sp-core/src/build/formula/source/meson.rs
...
pub fn meson_build(
    ...
    let meson_build_subdir_name = "sp-meson-build";
    ...
    let meson_exe =
        which::which_in("meson", build_env.get_path_string(), build_dir).map_err(|_| {
            SpError::BuildEnvError("meson command not found in build environment PATH.".to_string())
        })?;
    ...
    let _ninja_exe = which::which_in("ninja", build_env.get_path_string(), build_dir)
        .map_err(|_| SpError::BuildEnvError("ninja command not found.".to_string()))?;
