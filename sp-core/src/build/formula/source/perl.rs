// FILE: sp-core/src/build/formula/source/perl.rs
...
pub fn perl_build(
    ...
    if configure_script.is_file() {
        ...
        let sh_exe = which::which_in("sh", build_env.get_path_string(), source_dir)
            .map_err(|_| SpError::BuildEnvError("sh command not found.".to_string()))?;
        ...
        let perl_exe = which::which_in("perl", build_env.get_path_string(), source_dir)
            .map_err(|_| SpError::BuildEnvError("perl command not found.".to_string()))?;
    ...
    let make_exe = which::which_in("make", build_env.get_path_string(), source_dir)
        .map_err(|_| SpError::BuildEnvError("make command not found.".to_string()))?;
