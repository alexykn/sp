...
impl Tap {
    ...
    pub fn new(name: &str) -> Result<Self> {
        ...
        if parts.len() != 2 {
            return Err(SpError::Generic(format!("Invalid tap name: {name}")));
        }
    ...
    pub fn update(&self) -> Result<()> {
        ...
        let repo = Repository::open(&self.path)
            .map_err(|e| SpError::Generic(format!("Failed to open tap repository: {e}")))?;

        // Fetch updates from origin
        let mut remote = repo
            .find_remote("origin")
            .map_err(|e| SpError::Generic(format!("Failed to find remote 'origin': {e}")))?;
        ...
        remote
            ...
            .map_err(|e| SpError::Generic(format!("Failed to fetch updates: {e}")))?;

        // Merge changes
        let fetch_head = repo
            .find_reference("FETCH_HEAD")
            .map_err(|e| SpError::Generic(format!("Failed to find FETCH_HEAD: {e}")))?;

        let fetch_commit = repo
            .reference_to_annotated_commit(&fetch_head)
            .map_err(|e| SpError::Generic(format!("Failed to get commit from FETCH_HEAD: {e}")))?;

        let analysis = repo
            .merge_analysis(&[&fetch_commit])
            .map_err(|e| SpError::Generic(format!("Failed to analyze merge: {e}")))?;
        ...
        if analysis.0.is_fast_forward() {
            let mut reference = repo
                ...
                .map_err(|e| SpError::Generic(format!("Failed to find master branch: {e}")))?;
            reference
                .set_target(fetch_commit.id(), "Fast-forward")
                .map_err(|e| SpError::Generic(format!("Failed to fast-forward: {e}")))?;
            repo.set_head("refs/heads/master")
                .map_err(|e| SpError::Generic(format!("Failed to set HEAD: {e}")))?;
            repo.checkout_head(Some(git2::build::CheckoutBuilder::default().force()))
                .map_err(|e| SpError::Generic(format!("Failed to checkout: {e}")))?;
        } else {
    ...
    pub fn remove(&self) -> Result<()> {
        ...
        std::fs::remove_dir_all(&self.path).map_err(|e| {
            SpError::Generic(format!("Failed to remove tap {}: {}", self.full_name(), e))
        })
    }
