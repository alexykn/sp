...
impl From<reqwest::Error> for SpError {
    fn from(err: reqwest::Error) -> Self {
        SpError::Http(Arc::new(err))
    }
}

impl From<serde_json::Error> for SpError {
    fn from(err: serde_json::Error) -> Self {
        SpError::Json(Arc::new(err))
    }
}

impl From<semver::Error> for SpError {
    fn from(err: semver::Error) -> Self {
        SpError::SemVer(Arc::new(err))
    }
}

impl From<object::read::Error> for SpError {
    fn from(err: object::read::Error) -> Self {
        SpError::Object(Arc::new(err))
    }
}
