...
use sph_common::cache::Cache;
use sph_common::config::Config;
use sph_common::dependency::{
    DependencyResolver, ResolutionContext, ResolutionStatus, ResolvedGraph,
};
use sph_common::error::{Result, SphError};
use sph_common::formulary::Formulary;
use sph_common::keg::KegRegistry;
use sph_common::model::Cask;
// --- Shared Data Structures ---

// Reusable enum to identify target type, potentially moved from sph-core if made public
// Or defined locally here if InstallTargetIdentifier from core isn't suitable.
// Assuming we use the one from core for now:
use sph_common::model::InstallTargetIdentifier;
use sph_common::model::formula::{Formula, FormulaDependencies};
use sph_core::build::{self};
use sph_core::installed::{InstalledPackageInfo, PackageType}; /* Needs implementing in
                                                                * sph-core */
use sph_core::uninstall as core_uninstall; // Alias for the new module
use sph_core::uninstall::UninstallOptions; // Needs implementing in sph-core
use sph_core::update_check::{self, UpdateInfo}; // Needs implementing in sph-core
use sph_net::fetch::api;
...
pub enum PipelineJobResult {
    ...
    InstallErr(String, PackageType, SphError),
    UpgradeErr(String, PackageType, String, SphError), // Include old version
    ReinstallErr(String, PackageType, SphError),
}
...
type PlanResult = Result<(Vec<PipelineJob>, Vec<(String, SphError)>, HashSet<String>)>;
...
impl PipelineExecutor {
    ...
    pub async fn execute_pipeline(
        ...
        if planned_jobs.is_empty() {
            if overall_errors.is_empty() {
                ...
                let final_error_msg = overall_errors
                    ...
                    .collect::<Vec<_>>()
                    .join("; ");
                return Err(SphError::InstallError(format!(
        ...
        if let Err(e) = pump_handle.await {
            ...
            overall_errors.push((
                ...
                SphError::Generic(format!("Worker coordination failed: {e}")),
            ));
        }
        ...
        if overall_errors.is_empty() {
            ...
            let final_error_msg = overall_errors
                ...
                .collect::<Vec<_>>()
                .join("; ");
            Err(SphError::InstallError(format!(
    ...
    async fn plan_package_operations(
        ...
        let mut errors: Vec<(String, SphError)> = Vec::new();
        ...
        match command_type {
            CommandType::Install => {
                ...
                for name in initial_targets {
                    ...
                    match sph_core::installed::get_installed_package(name, config).await? {
                ...
                for name in initial_targets {
                    ...
                    match sph_core::installed::get_installed_package(name, config).await? {
                        ...
                        None => {
                            ...
                            error!("✖ {msg}");
                            errors.push((name.clone(), SphError::NotFound(msg)));
                            processed.insert(name.clone());
                        }
                    }
                }
            ...
            CommandType::Upgrade { all } => {
                ...
                let packages_to_check = if all {
                    sph_core::installed::get_installed_packages(config).await?
                } else {
            ...
            match opt_def {
                ...
                None => {
                    ...
                    if !errors.iter().any(|(n, _)| n == name) {
                        ...
                        error!("✖ {msg}");
                        errors.push((name.clone(), SphError::Generic(msg)));
                    }
                    processed.insert(name.clone());
                }
            }
                    ...
                    if !formulae_for_resolution.contains_key(formula_dep) {
                            ...
                            .remove(formula_dep)
                        {
                            ...
                            _ => {
                                ...
                                if !errors.iter().any(|(n, _)| n == formula_dep) {
                                    errors.push((formula_dep.clone(), SphError::NotFound(msg)));
                                }
                            }
                        }
                    }
            ...
            match resolver.resolve_targets(&resolution_target_names) {
                ...
                Err(e) => {
                    ...
                    for name in resolution_target_names {
                        if !errors.iter().any(|(n, _)| n == &name) {
                            errors.push((name.clone(), SphError::DependencyError(e.to_string())));
                        }
                    }
        ...
        if let Some(graph) = resolved_formula_graph {
            ...
            for (token, cask_arc) in cask_deps_map {
                ...
                if !jobs.iter().any(|j| match &j.target {
                    ...
                    if sph_core::installed::get_installed_package(&token, config)
                        .await?
                        .is_none()
                    {
                        jobs.push(PipelineJob {
        ...
        for name in names {
            ...
            futures.spawn(async move {
                ...
                match api::get_formula(&name).await {
                    ...
                    Err(SphError::NotFound(_)) | Err(SphError::Api(_)) | Err(SphError::Http(_)) => {
        ...
        while let Some(result) = download_join_set.join_next().await {
            match result {
                ...
                Ok(Err(e)) => {
                    ...
                    let name = match &e {
                        SphError::DownloadError(n, _, _) => n.clone(),
                        _ => "[unknown]".to_string(),
                    };
    ...
    fn collect_results(result_rx: Receiver<PipelineJobResult>) -> Vec<(String, SphError)> {
        let mut install_errors: Vec<(String, SphError)> = Vec::new();
        ...
        match &job.target {
            InstallTargetIdentifier::Formula(formula) => {
                ...
                if let Some(parent_dir) = install_dir.parent() {
                    fs::create_dir_all(parent_dir).map_err(|e| SphError::Io(Arc::new(e)))?;
                }
                ...
                if job.is_source_build {
                    ...
                    let resolved_graph = job.resolved_graph.as_ref().ok_or_else(|| {
                        SphError::Generic("Missing resolved graph for source build".to_string())
                    })?;
...
async fn download_target_file(
    ...
    .map_err(|e| {
        ...
        if matches!(e, SphError::DownloadError(_, _, _)) {
            e
        } else {
            SphError::DownloadError(
    ...
    match cache.load_raw(filename) {
        Ok(data) => {
            ...
            serde_json::from_str(&data).map_err(|e| {
                ...
                SphError::Cache(format!("Failed parse cached {filename}: {e}"))
            })
        }
        Err(_) => {
            ...
            serde_json::from_str(&raw_data).map_err(|e| SphError::Json(Arc::new(e)))
        }
    }
}
