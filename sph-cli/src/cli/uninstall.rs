...
use sph_common::Cache;
use sph_common::config::Config;
use sph_common::error::{Result, SphError};
use sph_core::{PackageType, UninstallOptions, installed, uninstall as core_uninstall};
    ...
    pub async fn run(&self, config: &Config, _cache: Arc<Cache>) -> Result<()> {
        ...
        for name in names {
            ...
            if name.contains('/') || name.contains("..") {
                ...
                error!("✖ {msg}");
                errors.push((name.to_string(), SphError::Generic(msg)));
                continue;
            }
            ...
            match installed::get_installed_package(name, config).await? {
                ...
                None => {
                    ...
                    error!("✖ {msg}");
                    errors.push((name.to_string(), SphError::NotFound(msg)));
                    pb.finish_and_clear();
                }
            }
        }
