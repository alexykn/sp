...
use sph_common::error::{Result as spResult, SphError};
...
async fn main() -> spResult<()> {
    ...
    let config =
        Config::load().map_err(|e| SphError::Config(format!("Could not load config: {e}")))?;
    ...
    let env_filter = EnvFilter::builder()
        ...
        .with_env_var("SPH_LOG") // Allow overriding via env var
        .from_env_lossy();
    ...
    let cache = Arc::new(
        Cache::new(&config.cache_dir)
            .map_err(|e| SphError::Cache(format!("Could not initialize cache: {e}")))?,
    );
...
async fn check_and_run_auto_update(config: &Config, cache: Arc<Cache>) -> spResult<()> {
    ...
    if env::var("SPH_NO_AUTO_UPDATE").is_ok_and(|v| v == "1") {
        tracing::debug!("Auto-update disabled via SPH_NO_AUTO_UPDATE=1.");
        return Ok(());
    }

    // 2. Determine update interval
    let default_interval_secs: u64 = 86400; // 24 hours
    let update_interval_secs = env::var("SPH_AUTO_UPDATE_SECS")
        .ok()
    ...
    let timestamp_file = config.cache_dir.join(".sph_last_update_check");
