// ===== sph-core/src/build/cask/artifacts/installer.rs =====
...
fn validate_argument(arg: &str) -> Result<String> {
    ...
    if arg.starts_with("/") || arg.contains("..") || arg.contains("/") {
        return Err(SphError::Generic(format!("Invalid argument: {arg}")));
    }
    if !arg
        .chars()
                    ...
                    for inst in insts {
                        if let Some(inst_obj) = inst.as_object() {
                            ...
                            let status = cmd.status().map_err(|e| {
                                SphError::Generic(format!("Failed to spawn installer script: {e}"))
                            })?;
                            if !status.success() {
