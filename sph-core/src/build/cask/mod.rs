...
use sph_common::model::cask::{Cask, Sha256Field, UrlField};
...
pub async fn download_cask(cask: &Cask, cache: &Cache) -> Result<PathBuf> {
    let url_field = cask
        .url
        .as_ref()
        .ok_or_else(|| SphError::Generic(format!("Cask {} has no URL", cask.token)))?;
    ...
    let parsed = Url::parse(url_str)
        .map_err(|e| SphError::Generic(format!("Invalid URL '{url_str}': {e}")))?;
    sph_net::validation::validate_url(parsed.as_str())?;
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|e| SphError::Http(std::sync::Arc::new(e)))?;
    ...
    if !expected_sha256.is_empty() {
        match sph_net::validation::verify_checksum(&cache_path, expected_sha256) {
            Ok(_) => {
...
pub fn install_cask(cask: &Cask, download_path: &Path, config: &Config) -> Result<()> {
    ...
    if non_extensions.contains(&detected_extension.as_str()) {
        ...
        match infer::get_from_path(download_path) {
            ...
            Err(e) => {
                ...
                return Err(SphError::Io(std::sync::Arc::new(e)));
            }
        }
    } else {
        debug!(
        ...
        && crate::build::formula::source::RECOGNISED_SINGLE_FILE_EXTENSIONS.contains(&expected_ext)
    {
        ...
        if let Err(e) = sph_net::validation::verify_content_type(download_path, expected_ext) {
...
pub fn write_receipt(
    ...
    let timestamp = SystemTime::now()
        ...
        .map_err(|e: SystemTimeError| SphError::Generic(format!("System time error: {e}")))?
        .as_secs();
    ...
    if let Some(parent) = receipt_path.parent() {
        fs::create_dir_all(parent).map_err(|e| SphError::Io(std::sync::Arc::new(e)))?;
    }
    let mut file =
        fs::File::create(&receipt_path).map_err(|e| SphError::Io(std::sync::Arc::new(e)))?;
    serde_json::to_writer_pretty(&mut file, &receipt_data)
        .map_err(|e| SphError::Json(std::sync::Arc::new(e)))?;
    debug!(
...
pub fn write_cask_manifest(
    ...
    serde_json::to_writer_pretty(writer, &manifest_data).map_err(|e| {
        ...
        SphError::Json(std::sync::Arc::new(e))
    })?;
    debug!(
