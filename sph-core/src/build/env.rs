...
use sph_common::model::formula::FormulaDependencies;
use tracing::debug;
...
pub struct BuildEnvironment {
    ...
    /// The root installation directory forsph(e.g., /opt/homebrew or /usr/local).
    #[allow(dead_code)]
    sph_prefix: PathBuf,
...
impl BuildEnvironment {
    ...
    pub fn new<F: FormulaDependencies>(
        ...
        debug!("sph Prefix: {}", sph_prefix.display());
        debug!(
        ...
        let sph_bin = sph_prefix.join("bin");
        if sph_bin.is_dir() {
            path_dirs.insert(0, sph_bin);
        }
        let sph_sbin = sph_prefix.join("sbin");
        if sph_sbin.is_dir() {
            path_dirs.insert(0, sph_sbin);
        }
        debug!("Prependedsphbin/sbin to PATH list.");
        ...
        let final_path_string = std::env::join_paths(path_dirs.iter())
            .map_err(|e| SphError::BuildEnvError(format!("Failed to join PATH: {e}")))?
            .into_string()
            .map_err(|os_str| {
                SphError::BuildEnvError(format!(
        ...
        Ok(Self {
            ...
            sph_prefix: sph_prefix.to_path_buf(),
