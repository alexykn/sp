// FILE: sph-core/src/build/formula/bottle.rs
...
use sph_common::model::formula::{BottleFileSpec, Formula, FormulaDependencies};
use sph_net::fetch::oci;
use sph_net::validation::verify_checksum;
...
pub async fn download_bottle(
    ...
    if bottle_file_spec.url.is_empty() {
        return Err(SphError::DownloadError(
            formula.name.clone(),
            "".to_string(),
    ...
    fs::create_dir_all(&cache_dir).map_err(|e| SphError::Io(std::sync::Arc::new(e)))?;
    ...
    if is_oci_blob_url {
        ...
        match sph_net::fetch::http::fetch_formula_source_or_bottle(
            formula.name(),
            bottle_url_str,
...
pub fn install_bottle(bottle_path: &Path, formula: &Formula, config: &Config) -> Result<PathBuf> {
    ...
    if let Some(parent_dir) = install_dir.parent() {
        fs::create_dir_all(parent_dir).map_err(|e| {
            SphError::Io(std::sync::Arc::new(std::io::Error::new(
                e.kind(),
                format!(
...
// sph-core/src/build/formula/bottle.rs
    ...
    if formula.name().starts_with("python@") {
        ...
        if let (Some(major), Some(minor)) = (parts.next(), parts.next()) {
            ...
            let status_id = StdCommand::new("install_name_tool")
                ...
                .stderr(Stdio::null())
                .status()
                .map_err(|e| SphError::Io(Arc::new(e)))?; // Use Arc::new
            ...
            let run_change = |old: &str, new: &str, target: &Path| -> Result<()> {
                ...
                let output = StdCommand::new("install_name_tool")
                    ...
                    .map_err(|e| SphError::Io(Arc::new(e)))?;
                ...
                if !output.status.success() {
                    ...
                    // Ignore empty stderr
                    {
                        ...
                        // return Err(SphError::InstallError(format!("install_name_tool -change
                        // failed: {}", stderr.trim())));
                    } else {
...
fn original_relocation_scan_and_patch(
    ...
    for entry in WalkDir::new(install_dir).into_iter().filter_map(|e| e.ok()) {
                    ...
                    .is_some_and(|e| e == "dylib" || e == "so" || e == "bundle"))
        {
            match macho::patch_macho_file(path, &replacements) {
                ...
                Err(SphError::PathTooLongError(e)) => { // Specifically catch path too long
                    error!(
                ...
                Err(SphError::CodesignError(e)) => { // Specifically catch codesign errors
                     error!(
                ...
                Err(e @ SphError::MachOError(_))
                | Err(e @ SphError::Object(_))
                | Err(e @ SphError::Generic(_)) // Catch Generic errors from patch_macho too
                | Err(e @ SphError::Io(_)) => { // Catch IO errors from patch_macho
                    debug!(
    ...
    if permission_errors > 0 || macho_errors > 0 || io_errors > 0 {
        ...
        // return Err(SphError::InstallError(format!("Bottle relocation encountered errors in {}",
        // install_dir.display())));
    }
    Ok(())
}
...
fn codesign_path(target: &Path) -> Result<()> {
    ...
    let status = StdCommand::new("codesign")
        .args([
            ...
            target
                .to_str()
                .ok_or_else(|| SphError::Generic("Non‑UTF8 path for codesign".into()))?,
        ])
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .status()
        .map_err(|e| SphError::Io(Arc::new(e)))?;
    if !status.success() {
        return Err(SphError::CodesignError(format!(
            "codesign failed for {}",
            target.display()
        )));
    }
...
fn write_text_file_atomic(original_path: &Path, content: &str) -> Result<()> {
    ...
    fs::create_dir_all(dir).map_err(|e| SphError::Io(Arc::new(e)))?; // Use Arc::new
    ...
    temp_file.persist(original_path).map_err(|e| {
        ...
        SphError::Io(Arc::new(e.error)) // Use Arc::new
    })?;
