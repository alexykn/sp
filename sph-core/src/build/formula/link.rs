// ===== sph-core/src/build/formula/link.rs =====
use std::fs;
use std::io::Write;
...
use serde_json;
use sph_common::config::Config; // Import Config
...
fn remove_existing_link_target(path: &Path) -> Result<()> {
    match path.symlink_metadata() {
        ...
        Err(e) => {
            ...
            Err(SphError::Io(std::sync::Arc::new(e)))
        }
    }
}
...
fn write_install_manifest(installed_keg_path: &Path, symlinks_created: &[String]) -> Result<()> {
    ...
    match serde_json::to_string_pretty(&symlinks_created) {
        ...
        Err(e) => {
            ...
            return Err(SphError::Json(std::sync::Arc::new(e)));
        }
    }
    Ok(())
}
...
fn is_executable(path: &Path) -> Result<bool> {
    ...
    if cfg!(unix) {
        ...
        match fs::metadata(path) {
            ...
            Err(e) => Err(SphError::Io(std::sync::Arc::new(e))),
        }
    } else {
        Ok(true)
    }
