// sph-core/src/build/formula/macho.rs
...
use sph_common::error::Result; // Keep top-level Result
#[cfg(target_os = "macos")]
use sph_common::error::SphError;
...
fn write_patched_buffer(original_path: &Path, buffer: &[u8]) -> Result<()> {
    ...
    fs::create_dir_all(dir).map_err(|e| SphError::Io(std::sync::Arc::new(e)))?;
    ...
    temp_file.persist(original_path).map_err(|e| {
        ...
        SphError::Io(std::sync::Arc::new(e.error))
    })?;
    debug!(
...
fn resign_binary(path: &Path) -> Result<()> {
    ...
    let status = StdCommand::new("codesign")
        ...
        .map_err(|e| {
            ...
            SphError::Io(std::sync::Arc::new(e))
        })?;
    if status.success() {
        ...
        Err(SphError::CodesignError(format!(
