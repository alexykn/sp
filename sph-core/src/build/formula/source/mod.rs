// sph-core/src/build/formula/source/mod.rs
...
use sph_common::model::formula::{Formula, FormulaDependencies, ResourceSpec};
use sph_net::fetch::http as http_fetch;
    ...
    match fs::read_dir(build_dir) {
        Ok(entries) => {
            ...
            for entry in entries.flatten() {
                ...
                if file_name_string.starts_with('.') || file_name_string == ".sph-resources" {
                    continue;
                }
        ...
        Err(e) => {
            return Err(SphError::Io(std::sync::Arc::new(std::io::Error::new(
                e.kind(),
        ...
        Some(kind) => {
            ...
            if SUPPORTED_ARCHIVE_EXTENSIONS.contains(&ext) {
                SUPPORTED_ARCHIVE_EXTENSIONS
                    ...
                    .copied()
                    .ok_or_else(|| {
                        SphError::Generic(format!("Internal error matching inferred ext {ext}"))
                    })
            } else {
                Err(SphError::Generic(format!(
        ...
        None => {
            ...
            if SUPPORTED_ARCHIVE_EXTENSIONS.contains(&ext) {
                SUPPORTED_ARCHIVE_EXTENSIONS
                    ...
                    .copied()
                    .ok_or_else(|| {
                        SphError::Generic(format!("Internal error matching file ext {ext}"))
                    })
            } else {
                Err(SphError::Generic(format!(
...
pub async fn build_from_source(
    ...
    if !expected_ext.is_empty() && RECOGNISED_SINGLE_FILE_EXTENSIONS.contains(&expected_ext) {
        ...
        if let Err(e) = sph_net::validation::verify_content_type(source_path, expected_ext) {
    ...
    let temp_build_dir = tempfile::Builder::new()
        ...
        .map_err(|e| SphError::IoError(format!("Failed create temp build dir: {e}")))?;
    ...
    if !resources.is_empty() {
        ...
        let resource_staging_base = build_dir.join(".sph-resources");
        ...
        let download_futures = resources.iter().map(|resource| {
            ...
            async move {
                ...
                let path = http_fetch::fetch_resource(&formula_name_clone, resource, &config_clone)
                    .await?;
                Ok::<_, SphError>((resource.name.clone(), path))
            }
        });
    ...
    let sph_prefix = config.prefix();
    let build_env =
        BuildEnvironment::new(formula, sph_prefix, &config.cellar, all_installed_paths)?;
...
fn install_perl_resource(
    ...
    let perl_exe = which::which_in("perl", build_env.get_path_string(), stage_path)
        .map_err(|_| SphError::BuildEnvError("perl not found.".to_string()))?;
    let make_exe = which::which_in("make", build_env.get_path_string(), stage_path)
        .map_err(|_| SphError::BuildEnvError("make not found.".to_string()))?;
...
fn install_python_resource(
    ...
    let python_exe = which::which_in("python3", build_env.get_path_string(), stage_path)
        ...
        .map_err(|_| SphError::BuildEnvError("python not found.".to_string()))?;
    ...
    if python_version_str.is_empty() {
        return Err(SphError::BuildEnvError(
...
fn install_single_file(source_path: &Path, formula: &Formula, install_dir: &Path) -> Result<()> {
    ...
    let target_filename = if formula.name == "ca-certificates" {
        ...
        source_path
            .file_name()
            .ok_or_else(|| SphError::Generic("Source path has no filename.".to_string()))?
    };
...
fn run_command_in_dir(
    ...
    let output = cmd.output().map_err(|e| {
        SphError::CommandExecError(format!(
    ...
    if !output.status.success() {
        ...
        Err(SphError::CommandExecError(format!(
