...
use sph_common::model::formula::ResourceSpec;
...
pub async fn fetch_formula_source_or_bottle(
    ...
    let mut last_error: Option<SphError> = None;
...
fn build_http_client() -> Result<Client> {
    ...
    Client::builder()
        ...
        .build()
        .map_err(|e| SphError::HttpError(format!("Failed to build HTTP client: {e}")))
}
...
async fn download_and_verify(
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|e| SphError::HttpError(format!("HTTP request failed for {url}: {e}")))?;
    ...
    if !status.is_success() {
        ...
        return match status {
            StatusCode::NOT_FOUND => Err(SphError::DownloadError(
                final_path
                    .file_name()
            ...
            StatusCode::FORBIDDEN => Err(SphError::DownloadError(
                final_path
                    .file_name()
                    .map(|s| s.to_string_lossy().to_string())
                    .unwrap_or_default(),
            ...
            _ => Err(SphError::HttpError(format!(
    ...
    let content = response
        .bytes()
        .await
        .map_err(|e| SphError::HttpError(format!("Failed to read response body bytes: {e}")))?;
