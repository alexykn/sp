...
const USER_AGENT_STRING: &str = "sph package manager (Rust; +https://github.com/alexykn/sp)";
...
async fn fetch_oci_resource<T: serde::de::DeserializeOwned>(
    ...
    let url = Url::parse(resource_url)
        .map_err(|e| SphError::Generic(format!("Invalid URL '{resource_url}': {e}")))?;
    ...
    let txt = resp.text().await.map_err(|e| SphError::Http(Arc::new(e)))?;
...
pub async fn download_oci_blob(
    ...
    let url = Url::parse(blob_url)
        .map_err(|e| SphError::Generic(format!("Invalid URL '{blob_url}': {e}")))?;
    crate::validation::validate_url(url.as_str())?;
    let registry_domain = url.host_str().unwrap_or(DEFAULT_GHCR_DOMAIN);
    let repo_path = extract_repo_path_from_url(&url).unwrap_or("");
    ...
    let mut out = File::create(&tmp).map_err(|e| SphError::Io(Arc::new(e)))?;

    let mut stream = resp.bytes_stream();
    while let Some(chunk) = stream.next().await {
        let b = chunk.map_err(|e| SphError::Http(Arc::new(e)))?;
        std::io::Write::write_all(&mut out, &b).map_err(|e| SphError::Io(Arc::new(e)))?;
    }
    std::fs::rename(&tmp, destination_path).map_err(|e| SphError::Io(Arc::new(e)))?;
...
pub fn build_oci_client() -> Result<Client> {
    Client::builder()
        ...
        .build()
        .map_err(|e| SphError::Http(Arc::new(e)))
}
        ...
        match client.get(&token_url).send().await {
            Ok(resp) if resp.status().is_success() => {
                let tok: OciTokenResponse = resp
                    .json()
                    .await
                    .map_err(|e| SphError::ApiRequestError(format!("Parse token response: {e}")))?;
                return Ok(tok.token);
            }
            Ok(resp) => {
                ...
                if !code.is_server_error() || attempt == MAX_RETRIES {
                    return Err(SphError::Api(format!("Token endpoint {code}: {body}")));
                }
            }
            Err(e) => {
                error!("Network error on token fetch {}: {}", attempt + 1, e);
                if attempt == MAX_RETRIES {
                    return Err(SphError::Http(Arc::new(e)));
                }
            }
        }
...
async fn execute_oci_request(
    ...
    let resp = req.send().await.map_err(|e| SphError::Http(Arc::new(e)))?;
    ...
    if status.is_success() {
        ...
        let err = match status {
            StatusCode::UNAUTHORIZED => SphError::Api(format!("Auth required: {status}")),
            StatusCode::FORBIDDEN => SphError::Api(format!("Permission denied: {status}")),
            StatusCode::NOT_FOUND => SphError::NotFound(format!("Not found: {status}")),
            _ => SphError::Api(format!("HTTP {status} – {body}")),
        };
        Err(err)
    }
}
