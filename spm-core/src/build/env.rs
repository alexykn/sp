...
pub struct BuildEnvironment {
    ...
    /// The root installation directory for spm (e.g., /opt/homebrew or /usr/local).
    #[allow(dead_code)]
    spm_prefix: PathBuf,
...
impl BuildEnvironment {
    ...
    pub fn new<F: FormulaDependencies>(
        ...
        debug!("spm Prefix: {}", spm_prefix.display());
        debug!(
        ...
        let spm_bin = spm_prefix.join("bin");
        if spm_bin.is_dir() {
            path_dirs.insert(0, spm_bin);
        }
        let spm_sbin = spm_prefix.join("sbin");
        if spm_sbin.is_dir() {
            path_dirs.insert(0, spm_sbin);
        }
        debug!("Prepended spm bin/sbin to PATH list.");
        ...
        let final_path_string = std::env::join_paths(path_dirs.iter())
            .map_err(|e| SpmError::BuildEnvError(format!("Failed to join PATH: {e}")))?
            .into_string()
            .map_err(|os_str| {
                SpmError::BuildEnvError(format!(
        ...
        Ok(Self {
            ...
            spm_prefix: spm_prefix.to_path_buf(),
