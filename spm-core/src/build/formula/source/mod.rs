// spm-core/src/build/formula/source/mod.rs
    ...
    match fs::read_dir(build_dir) {
        Ok(entries) => {
            ...
            for entry in entries.flatten() {
                ...
                if file_name_string.starts_with('.') || file_name_string == ".spm-resources" {
                    continue;
                }
        ...
        Some(kind) => {
            ...
            if SUPPORTED_ARCHIVE_EXTENSIONS.contains(&ext) {
                SUPPORTED_ARCHIVE_EXTENSIONS
                    ...
                    .copied()
                    .ok_or_else(|| {
                        SpmError::Generic(format!("Internal error matching inferred ext {ext}"))
                    })
            } else {
        ...
        None => {
            ...
            if SUPPORTED_ARCHIVE_EXTENSIONS.contains(&ext) {
                SUPPORTED_ARCHIVE_EXTENSIONS
                    ...
                    .copied()
                    .ok_or_else(|| {
                        SpmError::Generic(format!("Internal error matching file ext {ext}"))
                    })
            } else {
                Err(SpmError::Generic(format!(
...
pub async fn build_from_source(
    ...
    let temp_build_dir = tempfile::Builder::new()
        ...
        .map_err(|e| SpmError::IoError(format!("Failed create temp build dir: {e}")))?;
    ...
    if !resources.is_empty() {
        ...
        let resource_staging_base = build_dir.join(".spm-resources");
        ...
        let download_futures = resources.iter().map(|resource| {
            ...
            async move {
                ...
                let path = http_fetch::fetch_resource(&formula_name_clone, resource, &config_clone)
                    .await?;
                Ok::<_, SpmError>((resource.name.clone(), path))
            }
        });
    ...
    let spm_prefix = config.prefix();
    let build_env =
        BuildEnvironment::new(formula, spm_prefix, &config.cellar, all_installed_paths)?;
...
fn install_perl_resource(
    ...
    let perl_exe = which::which_in("perl", build_env.get_path_string(), stage_path)
        .map_err(|_| SpmError::BuildEnvError("perl not found.".to_string()))?;
    let make_exe = which::which_in("make", build_env.get_path_string(), stage_path)
        .map_err(|_| SpmError::BuildEnvError("make not found.".to_string()))?;
...
fn install_python_resource(
    ...
    let python_exe = which::which_in("python3", build_env.get_path_string(), stage_path)
        ...
        .map_err(|_| SpmError::BuildEnvError("python not found.".to_string()))?;
    ...
    if python_version_str.is_empty() {
        return Err(SpmError::BuildEnvError(
...
fn install_single_file(source_path: &Path, formula: &Formula, install_dir: &Path) -> Result<()> {
    ...
    let target_filename = if formula.name == "ca-certificates" {
        ...
        source_path
            .file_name()
            .ok_or_else(|| SpmError::Generic("Source path has no filename.".to_string()))?
    };
...
fn run_command_in_dir(
    ...
    let output = cmd.output().map_err(|e| {
        SpmError::CommandExecError(format!(
    ...
    if !output.status.success() {
        ...
        Err(SpmError::CommandExecError(format!(
