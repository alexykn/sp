        ...
        match client.get(&token_url).send().await {
            Ok(resp) if resp.status().is_success() => {
                let tok: OciTokenResponse = resp
                    .json()
                    .await
                    .map_err(|e| SpmError::ApiRequestError(format!("Parse token response: {e}")))?;
                return Ok(tok.token);
            }
            Ok(resp) => {
                ...
                if !code.is_server_error() || attempt == MAX_RETRIES {
                    return Err(SpmError::Api(format!("Token endpoint {code}: {body}")));
                }
            }
            Err(e) => {
...
async fn execute_oci_request(
    ...
    if status.is_success() {
        ...
        let err = match status {
            StatusCode::UNAUTHORIZED => SpmError::Api(format!("Auth required: {status}")),
            StatusCode::FORBIDDEN => SpmError::Api(format!("Permission denied: {status}")),
            StatusCode::NOT_FOUND => SpmError::NotFound(format!("Not found: {status}")),
            _ => SpmError::Api(format!("HTTP {status} – {body}")),
        };
        Err(err)
    }
}
