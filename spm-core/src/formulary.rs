...
use crate::utils::error::{Result, SpmError}; // Import the Cache struct // Import Arc for thread-safe shared ownership
...
impl Formulary {
    ...
    pub fn load_formula(&self, name: &str) -> Result<Formula> {
        ...
        let all_formulas: Vec<Formula> = serde_json::from_str(&raw_data)
            .map_err(|e| SpmError::Cache(format!("Failed to parse cached formula data: {e}")))?;
