// sps-core/src/build/formula/source/mod.rs
    ...
    match fs::read_dir(build_dir) {
        Ok(entries) => {
            ...
            for entry in entries.flatten() {
                ...
                if file_name_string.starts_with('.') || file_name_string == ".sps-resources" {
                    continue;
                }
        ...
        Some(kind) => {
            ...
            if SUPPORTED_ARCHIVE_EXTENSIONS.contains(&ext) {
                SUPPORTED_ARCHIVE_EXTENSIONS
                    ...
                    .copied()
                    .ok_or_else(|| {
                        SpsError::Generic(format!("Internal error matching inferred ext {ext}"))
                    })
            } else {
        ...
        None => {
            ...
            if SUPPORTED_ARCHIVE_EXTENSIONS.contains(&ext) {
                SUPPORTED_ARCHIVE_EXTENSIONS
                    ...
                    .copied()
                    .ok_or_else(|| {
                        SpsError::Generic(format!("Internal error matching file ext {ext}"))
                    })
            } else {
                Err(SpsError::Generic(format!(
...
pub async fn build_from_source(
    ...
    if !expected_ext.is_empty() && RECOGNISED_SINGLE_FILE_EXTENSIONS.contains(&expected_ext) {
        ...
        if let Err(e) = sps_net::validation::verify_content_type(source_path, expected_ext) {
    ...
    let temp_build_dir = tempfile::Builder::new()
        ...
        .map_err(|e| SpsError::IoError(format!("Failed create temp build dir: {e}")))?;
    ...
    crate::install::extract::extract_archive(
        source_path,
        build_dir,
        strip_components,
    ...
    if !resources.is_empty() {
        ...
        let resource_staging_base = build_dir.join(".sps-resources");
        ...
        let download_futures = resources.iter().map(|resource| {
            ...
            async move {
                ...
                let path = http_fetch::fetch_resource(&formula_name_clone, resource, &config_clone)
                    .await?;
                Ok::<_, SpsError>((resource.name.clone(), path))
            }
        });
...
fn install_perl_resource(
    ...
    let perl_exe = which::which_in("perl", build_env.get_path_string(), stage_path)
        .map_err(|_| SpsError::BuildEnvError("perl not found.".to_string()))?;
    let make_exe = which::which_in("make", build_env.get_path_string(), stage_path)
        .map_err(|_| SpsError::BuildEnvError("make not found.".to_string()))?;
...
fn install_python_resource(
    ...
    let python_exe = which::which_in("python3", build_env.get_path_string(), stage_path)
        ...
        .map_err(|_| SpsError::BuildEnvError("python not found.".to_string()))?;
...
fn install_single_file(source_path: &Path, formula: &Formula, install_dir: &Path) -> Result<()> {
    ...
    let target_filename = if formula.name == "ca-certificates" {
        ...
        source_path
            .file_name()
            .ok_or_else(|| SpsError::Generic("Source path has no filename.".to_string()))?
    };
