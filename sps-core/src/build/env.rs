...
pub struct BuildEnvironment {
    ...
    /// The root installation directory forsps(e.g., /opt/homebrew or /usr/local).
    #[allow(dead_code)]
    sps_prefix: PathBuf,
...
impl BuildEnvironment {
    ...
    pub fn new<F: FormulaDependencies>(
        ...
        debug!("sps Prefix: {}", sps_prefix.display());
        debug!(
        ...
        let sps_bin = sps_prefix.join("bin");
        if sps_bin.is_dir() {
            path_dirs.insert(0, sps_bin);
        }
        let sps_sbin = sps_prefix.join("sbin");
        if sps_sbin.is_dir() {
            path_dirs.insert(0, sps_sbin);
        }
        debug!("Prependedspsbin/sbin to PATH list.");
        ...
        let final_path_string = std::env::join_paths(path_dirs.iter())
            .map_err(|e| SpsError::BuildEnvError(format!("Failed to join PATH: {e}")))?
            .into_string()
            .map_err(|os_str| {
                SpsError::BuildEnvError(format!(
        ...
        Ok(Self {
            ...
            sps_prefix: sps_prefix.to_path_buf(),
