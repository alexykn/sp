    ...
    match &job_request.target_definition {
        ...
        InstallTargetIdentifier::Cask(cask) => {
            if is_source_from_private_store {
                debug!("[{}] Reinstalling cask from private store...", job_request.target_id);
                ...
                if let Some(file_name) = download_path.file_name() {
                    ...
                    if applications_app_path.exists() || applications_app_path.symlink_metadata().is_ok() {
                        debug!("Removing existing app at {}", applications_app_path.display());
                        let _ = build::cask::helpers::remove_path_robustly(&applications_app_path, config, true);
                    }
                    
                    // Copy from private store to /Applications using cp -Rp to preserve attributes
                    debug!("Copying app from private store {} to {}", download_path.display(), applications_app_path.display());
                    ...
                    #[cfg(target_os = "macos")]
                    {
                        if let Err(e) = crate::macos::xattr::set_quarantine_attribute(&applications_app_path, &cask.token) {
                            return Err(e);
                        }
                    }
                    ...
                    if caskroom_symlink_path.exists() || caskroom_symlink_path.symlink_metadata().is_ok() {
                        let _ = fs::remove_file(&caskroom_symlink_path);
                    }
                    
                    #[cfg(unix)]
                    {
                        if let Err(e) = std::os::unix::fs::symlink(&applications_app_path, &caskroom_symlink_path) {
